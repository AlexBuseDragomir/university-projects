...
// is using Prim's algorithm as a heuristic
...
public class RBFSprim
{
    ...
    public RBFSprim(GraphCostMatrix myGraph, Node currentNode)
    {
    ...
    public boolean solveRBFS(Node currentNode, int fLimit)
    {
        ...
        for(int neighbourIndex = 1; neighbourIndex <= this.numberOfNodes; neighbourIndex++)
        {
            ...
            {
                // we apply Prim's algorithm to the graph (only the unvisited nodes)
                // see constructor of Prim.java (there are kept only the edge costs for unvisited nodes)
                Prim minimumSpanningTree = new Prim(this.costMatrix, this.isVisited, this.numberOfNodes);
                // we retrieve the heuristic cost - the cost of all the edges of the minimum spanning tree
